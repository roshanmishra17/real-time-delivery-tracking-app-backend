...
def get_all_agents_latest_locations(
    admin=Depends(require_admin),
    db: Session = Depends(get_db)
):

    # Step 1: Get latest log per agent
    subquery = (
        db.query(
            models.LocationLog.agent_id,
            models.LocationLog.order_id,
            models.LocationLog.lat,
            models.LocationLog.lng,
            models.LocationLog.timestamp
        )
        .order_by(models.LocationLog.agent_id, models.LocationLog.timestamp.desc())
        .distinct(models.LocationLog.agent_id)
        .all()
    )
